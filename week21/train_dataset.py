...
class TrainDataset(Dataset):
    ...
    def generate_triplets(self, df, num_triplets,training_triplets_path):
        ...
        for _ in progress_bar:
            ...
            # 选出这个类对应的人名
            pos_name = str(df.loc[df['class'] == pos_class, 'name'].values[0])
            neg_name = str(df.loc[df['class'] == neg_class, 'name'].values[0])
    ...
    def add_extension(self, path):
        # 文件格式比较迷，可能有这两种情况
        if os.path.exists(path + r'.jpg'):
            return str(path + '.jpg')
        elif os.path.exists(path + r'.png'):
            return str(path + '.png')
        else:
    ...
    def __getitem__(self, idx):
		...
		# 要加上编码形式的修改
        anc_img = self.add_extension(os.path.join(self.face_dir, str(pos_name, encoding='utf-8'), str(anc_id, encoding='utf-8')))
        pos_img = self.add_extension(os.path.join(self.face_dir, str(pos_name, encoding='utf-8'), str(pos_id, encoding='utf-8')))
        neg_img = self.add_extension(os.path.join(self.face_dir, str(neg_name, encoding='utf-8'), str(neg_id, encoding='utf-8')))
